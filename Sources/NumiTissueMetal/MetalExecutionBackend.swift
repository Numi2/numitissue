...
private final class MetalPhaseHeaderRing: @unchecked Sendable {
    ...
    init(context: MetalDeviceContext, capacity: Int = 2_048) throws {
        ...
        self.buffer = try context.makeSharedBuffer(length: capacity * stride, label: "NumiTissue.phaseHeaders", writeCombined: true)
    }
...
public actor MetalTissueBackend: InterventionAwareTissueBackend {
    ...
    public func execute(phase: RuntimePhase, tickRange: Range<UInt64>, context executionContext: ExecutionContext) async throws {
        ...
        guard let table = argumentTables[ObjectIdentifier(arena.shadow)] else { throw MetalRuntimeError.stateNotLoaded }

        var header = MetalSimulationHeader(state: arena.committedCPUState, context: executionContext, phase: phase, phaseRange: tickRange)
        ...
        switch phase {
        case .ingestInputs:
            try encode(kernel: .ingestInputEvents, count: max(Int(header.reserved0.x), Int(header.reserved0.y)), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        case .buildWorklists:
            try encode(kernel: .buildWorklists, count: Int(header.tileCount), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
            try encode(kernel: .encodeIndirectDispatch, count: 7, header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        case .deliverEvents:
        ...
        case .detectSpikes:
            try encode(kernel: .detectSpikes, count: Int(header.compartmentCount), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        case .routeSpikes:
        ...
        case .collectOutputs:
            try encode(kernel: .collectOutputs, count: max(Int(header.tileCount), Int(header.eventCapacity)), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        case .validate:
            let count = max(Int(header.cellCount), Int(header.compartmentCount), Int(header.synapseCount), Int(header.fieldValueCount), Int(header.microdomainCount), Int(header.molecularSpeciesCount))
            try encode(kernel: .validateState, count: count, header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        }
    }
    ...
    public func collectOutput(context executionContext: ExecutionContext) async throws -> RuntimeOutputFrame {
        ...
        let pointer = arena.transient.outputEvents.contents().bindMemory(to: MetalEvent.self, capacity: max(generated, 1))
        var output = RuntimeOutputFrame(startTime: executionContext.startTime, endTime: executionContext.endTime)
        output.efferentEvents.reserveCapacity(generated)
        for index in 0..<generated { output.efferentEvents.append(pointer[index].routedEvent) }

        let tileCount = arena.committedCPUState.tiles.count
        let scalars = arena.transient.outputScalars.contents().bindMemory(to: Float.self, capacity: max(16 + tileCount * 4, 1))
        ...
        if damage > 0.5 {
            output.damageEvents.append(RoutedEvent(arrivalTick: executionContext.endTime.tick, source: 0, destination: 0, amplitude: damage, kind: .damage))
        }
        return output
    }
    ...
    public func validateShadow(context: ExecutionContext) async throws -> [RuntimeValidationIssue] {
        ...
        let records = arena.transient.validationRecords.contents().bindMemory(to: MetalValidationRecord.self, capacity: max(count, 1))
    ...
    private func encodeCableSolve(
        ...
        try encode(kernel: .assembleCableSystem, count: Int(header.compartmentCount), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        if maxCableDepth > 0 {
            for depth in stride(from: maxCableDepth, through: 1, by: -1) {
                var levelHeader = header
                levelHeader.reserved2.x = depth
                try encode(kernel: .eliminateCableLevels, count: Int(header.compartmentCount), header: levelHeader, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
            }
        }
        try encode(kernel: .solveCableRoots, count: Int(header.compartmentCount), header: header, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
        if maxCableDepth > 0 {
            for depth in 1...maxCableDepth {
                var levelHeader = header
                levelHeader.reserved2.x = depth
                try encode(kernel: .backSubstituteCableLevels, count: Int(header.compartmentCount), header: levelHeader, command: command, ring: ring, table: table, state: arena.shadow, transient: arena.transient)
            }
        }
    }
    ...
    private func encode(
        ...
        guard let blit = command.makeBlitCommandEncoder() else { throw MetalRuntimeError.encoderCreationFailed("phaseHeader") }
        blit.label = "NumiTissue.header.\(kernel.rawValue)"
        blit.copy(from: ring.buffer, sourceOffset: offset, to: transient.header, destinationOffset: 0, size: MemoryLayout<MetalSimulationHeader>.stride)
        blit.endEncoding()
    ...
    private func clearPersistentTransientState(arena: MetalStateArena, shaders: MetalShaderLibrary, table: MetalArgumentTable) async throws {
        ...
        guard let encoder = command.makeComputeCommandEncoder() else { throw MetalRuntimeError.encoderCreationFailed("reset") }
    ...
    private static func validationMessage(code: UInt32, index: UInt32) -> String {
        switch code {
        case ValidationCode.nonFinite: return "Non-finite GPU state at index \(index)"
        case ValidationCode.negativeConcentration: return "Negative concentration at index \(index)"
        case ValidationCode.invalidTopology: return "Invalid packed topology at index \(index)"
        case ValidationCode.eventOverflow: return "GPU event bucket overflow"
        case ValidationCode.voltageBounds: return "Membrane voltage outside configured bounds"
        case ValidationCode.positiveCellVolume: return "Cell geometry outside configured bounds"
        case ValidationCode.metabolicBounds: return "Metabolic or damage state outside configured bounds"
        case ValidationCode.weightBounds: return "Synaptic weight outside configured bounds"
        default: return "NumiTissue GPU validation code \(code) at index \(index)"
        }
    }
}
