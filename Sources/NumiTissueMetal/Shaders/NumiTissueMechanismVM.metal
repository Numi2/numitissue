...
inline void nt_emit_event(
    ...
    if (result.eventCount >= maximumEvents) {
        result.fault = NTFaultEventOverflow;
        return;
    }
    const uint eventIndex = instanceIndex * maximumEvents + result.eventCount;
        ...
        switch (instruction.opcode) {
            ...
            case NTCall:
                ...
                if (frameDepth >= min(program.maximumCallDepth, 8u)) { output.fault = NTFaultCallDepth; break; }
                {
                    ...
                    if (output.fault != NTFaultNone) { break; }
                    frames[frameDepth++] = { pc, activeEnd, sp, instruction.flags & 1u };
            ...
            case NTEmitNetEvent:
                if (!nt_pop(stack, sp, value)) { output.fault = NTFaultStackUnderflow; break; }
                nt_emit_event(events, instanceIndex, maximumEvents, output, 1u, 0, value);
                break;
            case NTWatch:
                if (!nt_pop(stack, sp, value)) { output.fault = NTFaultStackUnderflow; break; }
                if (value != 0.0f) { nt_emit_event(events, instanceIndex, maximumEvents, output, 2u, int(instruction.operandA), value); }
                break;
            case NTEnd:
                pc = activeEnd;
                break;
...
inline uint nt_run_entry(
    ...
    while (pc < end) {
        NTVMResult result = nt_run_vm(
            ...
            events, instanceIndex, maximumEvents - min(eventCount, maximumEvents), instructionBudget, instructionCount
        );
        ...
        NTVMResult derivative = nt_run_vm(
        ...
        instructionCount = derivative.instructionCount;
        eventCount += derivative.eventCount;
        if (derivative.fault != NTFaultNone) { return derivative.fault; }
        ...
        for (uint i = 0u; i < program.integratorCount; ++i) {
            ...
            if (perturbed.fault != NTFaultNone) { return perturbed.fault; }
...
kernel void nt_mechanism_execute(
    ...
    const uint maximumEvents = parameters.counts.y;
    ...
    if (mode == 0u) {
        fault = nt_run_entry(
            ...
            events, instanceIndex, maximumEvents, instructionBudget, instructionCount, eventCount
        );
    } else {
        fault = nt_run_entry(
